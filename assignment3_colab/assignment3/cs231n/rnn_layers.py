...
def rnn_step_backward(dnext_h, cache):
    ...
    # 여기서 a = x*Wx + prev_h * Wh + b 라고 했을 때,
    # dnext_h = dl/da가 된다.

    dforward = (1- np.tanh(forward)**2) * dnext_h
    dx = np.dot(dforward, Wx.T) # dl/dx = dl/da * da/dx이므로 dot product를 통해 dl/dx가 나오게 된다.
    dWx = np.dot(x.T, dforward) # dl/dWx = dl/da * da/dWx
    dprev_h = np.dot(dforward, Wh)
...
def lstm_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
        ...
        dc_current = dc_prev
        dx_t, dWx_t, dc_prev, dWh_t, dh_prev, db_t = lstm_step_backward(dh_current, dc_current,cache[t])
